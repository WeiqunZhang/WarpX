...
: m_nr(nr), m_nk(nr)
{
    ...
    if (azimuthal_mode !=0 && hankel_order != azimuthal_mode-1) {
        ...
        amrex::Vector<Real> invMcopy(invM);
        amrex::Vector<Real> sdiag(m_nk-1, 0.);
        amrex::Vector<Real> u((m_nk-1)*(m_nk-1), 0.);
        amrex::Vector<Real> vt((m_nr)*(m_nr), 0.);
        amrex::Vector<Real> sp((m_nr)*(m_nk-1), 0.);
        amrex::Vector<Real> temp((m_nr)*(m_nk-1), 0.);
